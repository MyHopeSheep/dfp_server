package com.dataframe.prase.infrastructure.excel;
...
public final class FrameParseExcelWriter {
    ...
    public void write(FrameParseResult outcome) throws IOException {
    ...
    private void writeResultsSheet(Workbook workbook, FrameParseResult outcome, Styles styles) {
        ...
        int rowIndex = 2;
        for (DecodedFrame frame : outcome.frames()) {
    ...
    private void writeFrameRow(Row row, DecodedFrame frame, Styles styles) {
        ...
        FrameDecodeAudit audit = frame.audit();
    ...
    private String protocolByte(DecodedFrame frame, int coreByteIndex) {
        if (!frame.headerLocated() || coreByteIndex >= frame.recoveredBytes().size()) {
            return "未恢复";
        }
        return "0x" + DecodedFrame.hex(frame.recoveredBytes().get(coreByteIndex));
    }

    private void writeSummarySheet(Workbook workbook, FrameParseResult outcome, Styles styles) {
        var rules = outcome.configuration().protocolRuleSet();
        ...
        rowIndex = summaryRow(sheet, rowIndex, "合法单bit脉宽窗口(us)",
                derived.singleBitPulseMinUs().toPlainString()
                        + "～" + derived.singleBitPulseMaxUs().toPlainString(), styles);
        rowIndex = summaryRow(sheet, rowIndex, "周期估计方式",
                "同步前导连续边沿初步拟合，确认" + rules.frame().syncMarkerHex()
                        + "后按已知跳变位置细化拟合", styles);
        rowIndex = summaryRow(sheet, rowIndex, "采样相位步长", derived.phaseStepExpression(), styles);
        rowIndex = summaryRow(sheet, rowIndex, "同一物理帧去重容差",
                "两个候选中较大T_est的" + formatRatio(rules.timing().dedupToleranceRatio()),
                styles);
        ...
        for (UnrecognizedSignalRange fragment : outcome.boundaryFragments()) {
            Row row = sheet.createRow(rowIndex++);
    ...
    private String formatRatio(BigDecimal ratio) {
        ...
        BigInteger denominator = BigInteger.TEN.pow(normalized.scale());
